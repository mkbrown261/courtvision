// ═══════════════════════════════════════════════════════════════════
// GAME VISION — PERSISTENT DATA LAYER
// ═══════════════════════════════════════════════════════════════════

export type SportType = 'basketball' | 'soccer' | 'football' | 'hockey'

export type Position = string // flexible per sport
...
export type Coach = {
  ...
  teamIds: string[]
  createdAt: number
  role: 'coach' | 'scorekeeper' | 'admin'
}

export type GamePlayer = {
  id: string           // references SavedPlayer.id
...
export type Game = {
  ...
  location?: string
  notes?: string
  // playerId -> timestamp they subbed in
...
export function computeGameStats(gameId: string): {
  ...
  for (const p of allPlayers) {
    ...
    const teamName = isA ? game.teamA.name : game.teamB.name
    statsMap[p.id] = {
      ...
      shots: 0, powerPlays: 0,
      minutesPlayed: 0, isActive: false
    }
  }
...
export type StatAction = {
  ...
  icon: string
  color: string
  promptValue?: boolean   // ask for numeric value
  valueOptions?: number[] // preset options (e.g. 1/2/3 for pts)
  defaultValue?: number
}
...
export type SportConfig = {
  ...
  emoji: string
  actions: StatAction[]
  scoringStat: string   // which stat drives the score
  periods: string[]     // e.g. ['Q1','Q2','Q3','Q4']
