...
.player-row{display:flex;align-items:center;gap:12px;background:var(--bg3);border-radius:10px;padding:9px 12px;border:1px solid transparent;transition:border-color 0.15s}
...
.preview-list{margin-top:12px;display:flex;flex-direction:column;gap:6px;max-height:200px;overflow-y:auto}
.preview-item{display:flex;align-items:center;gap:10px;background:var(--bg4);border-radius:8px;padding:7px 10px;font-size:12px}
...
.voice-btn.recording{background:var(--danger);animation:voicePulse 1s infinite;box-shadow:0 0 0 12px rgba(255,71,87,0)}
...
.toast{background:var(--bg4);border:1px solid var(--border2);border-radius:12px;padding:11px 16px;font-size:13px;font-weight:500;display:flex;align-items:center;gap:9px;box-shadow:var(--shadow);pointer-events:auto;animation:tIn 0.3s ease;max-width:320px}
  ...
  <div class="shell">
    ...
    <div class="main-content">
      ...
      <div class="view" id="view-teams">
        <div class="scroll-area">
          <div class="sec-header">
            <div><div class="sec-title"><i class="fas fa-shield-halved"></i> Team Roster</div><div class="sec-sub">Persistent teams — set once, reuse forever</div></div>
      ...
      <div class="view" id="view-ingestion">
        <div class="scroll-area">
          ...
          <div class="ing-panel active" id="ing-csv">
            <div class="card">
              <div class="card-body">
                <p style="font-size:13px;color:var(--text2);margin-bottom:10px">Paste CSV, roster sheet text, or upload a file. Format: <code style="color:var(--primary)">#,Name,Position</code> or <code style="color:var(--primary)">Name,#</code></p>
                ...
                <div style="display:flex;gap:8px;margin-top:10px">
                  <button class="btn btn-primary" onclick="parseCSV()"><i class="fas fa-wand-magic-sparkles"></i> Parse Roster</button>
                  <button class="btn btn-ghost" onclick="document.getElementById('csv-text').value=''">Clear</button>
                </div>
                <div id="csv-preview" class="preview-list" style="margin-top:12px"></div>
                <div id="csv-import-actions" style="display:none;margin-top:12px"></div>
              </div>
            </div>
          </div>
          ...
          <div class="ing-panel" id="ing-voice">
            <div class="card">
              <div class="card-body" style="text-align:center">
                <p style="font-size:13px;color:var(--text2);margin-bottom:16px">Speak naturally: <em>"Team Eagles, number 23 John Smith, number 5 Marcus Lee..."</em></p>
                ...
                <div style="display:flex;gap:8px;margin-top:12px;justify-content:center">
                  <button class="btn btn-primary" onclick="parseVoice()"><i class="fas fa-wand-magic-sparkles"></i> Parse Players</button>
                  <button class="btn btn-ghost" onclick="clearVoice()">Clear</button>
                </div>
                <div id="voice-preview" class="preview-list" style="margin-top:12px"></div>
                <div id="voice-import-actions" style="display:none;margin-top:12px"></div>
              </div>
            </div>
          </div>
          ...
          <div class="ing-panel" id="ing-manual">
            <div class="card">
              <div class="card-body">
                <p style="font-size:13px;color:var(--text2);margin-bottom:14px">Quickly add players one by one. Select target team above.</p>
                <div class="form-row-3">
                  <div class="form-group"><label class="form-label">#</label><input class="form-input" type="text" id="m-jersey" placeholder="23" maxlength="3"/></div>
                ...
                <div style="display:flex;gap:8px">
                  ...
                  <button class="btn btn-ghost btn-sm" onclick="bulkSaveManual()"><i class="fas fa-save"></i> Save All to Team</button>
                </div>
          ...
          <div class="ing-panel" id="ing-photo">
            <div class="card">
              <div class="card-body" style="text-align:center">
                ...
                <p style="color:var(--text2);font-size:13px;margin-bottom:16px">Upload a photo of a roster sheet or score sheet. We'll attempt to extract player names and jersey numbers.</p>
                ...
                <p style="font-size:11px;color:var(--text3);margin-top:10px">⚠️ OCR accuracy depends on image quality. Always review before saving.</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="modal-body">
      ...
      <div class="roster-split">
        <div class="roster-panel pa">
          ...
          <div class="add-row">
            <input class="form-input" id="ng-a-j" placeholder="#" maxlength="3"/>
        ...
        <div class="roster-panel pb">
          ...
          <div class="add-row">
            <input class="form-input" id="ng-b-j" placeholder="#" maxlength="3"/>
...
async function loadDashboard() {
  ...
  // Top performers (load scores for recent)
  loadTopPerformers(d.recentGames || [])
}

function gameCardHTML(g, isLive = false) {
  const status = g.status === 'active' ? '<span class="badge badge-live"><i class="fas fa-circle" style="font-size:7px"></i> Live</span>'
  ...
  return \`<div class="game-card" onclick="openGame('\${g.id}')">
    ...
    <div class="game-meta">\${sport} \${esc(g.sport)} &bull; \${date} \${status} \${g.name ? '&bull; '+esc(g.name) : ''}</div>
  </div>\`
}

async function loadTopPerformers(recentGames) {
  const finished = recentGames.filter(g => g.status === 'finished')
  if (!finished.length) { document.getElementById('top-performers').innerHTML = '<p style="color:var(--text3);font-size:13px">Complete games will show top performers here.</p>'; return }
...
async function loadTeamsView() {
  ...
  if (!d.teams?.length) {
    container.innerHTML = '<div class="empty-state"><i class="fas fa-shield-halved"></i><h3>No teams yet</h3><p>Create a team to build a persistent roster.</p><button class="btn btn-primary" onclick="showCreateTeamModal()"><i class="fas fa-plus"></i> Create Team</button></div>'
    return
  }
...
function renderTeamRoster(team) {
  ...
  c.innerHTML = '<div class="player-list">' + team.players.map(p => \`
...
async function createTeam() {
  ...
  if (d.team) { toast('Team created!','ok'); closeModal('modal-create-team'); loadTeamsView(); refreshIngestionTargets(); refreshNewGameDropdowns() }
}
...
function showAddPlayerModal(teamId) {
  ...
  const pos = prompt('Position (optional):') || ''
  api('POST', \`/teams/\${teamId}/players\`, {name:name.trim(),jerseyNumber:jersey.trim(),position:pos||undefined})
    .then(() => { toast('Player added','ok'); openTeamDetail(teamId) })
}
...
async function refreshNewGameDropdowns() {
  const d = await api('GET', '/teams')
  const teams = d.teams || []
  const makeOpts = (sel) => '<option value="">— Enter manually —</option>' +
    teams.map(t => \`<option value="\${t.id}">\${esc(t.name)} (\${sportEmoji(t.sport)} \${t.players.length}pl)</option>\`).join('')
  const sa = document.getElementById('ng-team-a-saved')
  const sb = document.getElementById('ng-team-b-saved')
  if (sa) sa.innerHTML = makeOpts('A')
  if (sb) sb.innerHTML = makeOpts('B')
}
...
async function createGame() {
  ...
  if (!d.game) { toast('Failed to create game','err'); btn.innerHTML = '<i class="fas fa-play"></i> Create & Start Tracking'; btn.disabled = false; return }
  ...
  const tasks = [
    ...S.ngPlayersA.filter(p => !p.savedPlayerId).map(p => api('POST',\`/games/\${gid}/players\`,{name:p.name,jerseyNumber:p.jersey,team:'A'})),
    ...S.ngPlayersB.filter(p => !p.savedPlayerId).map(p => api('POST',\`/games/\${gid}/players\`,{name:p.name,jerseyNumber:p.jersey,team:'B'}))
  ]
...
function getConfig() {
  ...
  // Inline SPORT_CONFIGS mirror for client side
  const configs = {
    basketball: {
      actions: [
...
async function logStat(type, value = 1) {
  ...
  if (d.stats) {
    ...
    const labels = {POINTS:\`+\${value}pts\`,ASSIST:'+AST',REBOUND:'+REB',BLOCK:'+BLK',STEAL:'+STL',
      TURNOVER:'TOV',FOUL:'FOUL',GOAL:'+GOAL',YELLOW_CARD:'YC',RED_CARD:'RC',SAVE:'+SAVE',
      SHOT_ON_TARGET:'+SOT',TOUCHDOWN:'+TD',FIELD_GOAL:'+FG',TACKLE:'+TKL',SACK:'+SCK',
      INTERCEPTION:'+INT',SHOT:'+SHOT',SUB_IN:'SUB IN ✅',SUB_OUT:'SUB OUT ❌',
...
async function parseCSV() {
  ...
  const d = await api('POST', '/ingest/csv', { text })
  showIngPreview(d.players || [], 'csv')
}
...
function toggleVoice() {
  if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
    toast('Speech recognition not supported in this browser. Try Chrome.','err')
    return
  }
...
function clearVoice() {
  ...
  document.getElementById('voice-import-actions').style.display = 'none'
}
...
async function parseVoice() {
  ...
  const d = await api('POST', '/ingest/voice', { transcript })
  showIngPreview(d.players || [], 'voice')
}
...
function renderManualPreview() {
  ...
  c.innerHTML = S.manualPlayers.map((p,i) => \`
    <div class="preview-item">
      ...
      <span style="flex:1">\${esc(p.name)}\${p.position?' · '+esc(p.position):''}</span>
  ...
  \`).join('')
  if (S.manualPlayers.length > 0 && !document.getElementById('manual-save-btn')) {
    c.insertAdjacentHTML('afterend', '<div id="manual-save-wrap" style="margin-top:10px"><button id="manual-save-btn" class="btn btn-success" onclick="bulkSaveManual()"><i class="fas fa-save"></i> Save All to Team</button></div>')
  }
}

async function bulkSaveManual() {
  const teamId = document.getElementById('ingest-target-team').value
  const players = S.manualPlayers
  if (!players.length) { toast('Add players first','err'); return }

  if (teamId) {
    const d = await api('POST', \`/teams/\${teamId}/import\`, { players })
    toast('Saved ' + d.count + ' players to team!', 'ok')
  } else {
    const teamName = prompt('New team name:')
    if (!teamName) return
    const td = await api('POST', '/teams', { name:teamName.trim(), sport:'basketball', color:'#6C63FF', logoInitials:teamName.substring(0,2).toUpperCase() })
    if (td.team) {
      await api('POST', \`/teams/\${td.team.id}/import\`, { players })
      toast('Created "' + teamName + '" with ' + players.length + ' players!', 'ok')
    }
  }
  S.manualPlayers = []
  renderManualPreview()
  ...
  document.getElementById('manual-save-wrap')?.remove()
}

function showIngPreview(players, source) {
  const prev = document.getElementById(\`\${source}-preview\`)
  const acts = document.getElementById(\`\${source}-import-actions\`)
  if (!players.length) { prev.innerHTML = '<p style="color:var(--danger);font-size:13px;padding:8px">No players could be parsed. Check format.</p>'; acts.style.display='none'; return }

  prev.innerHTML = players.map(p => \`
    <div class="preview-item">
      <div class="preview-jersey">\${esc(p.jerseyNumber)}</div>
  ...
  acts.innerHTML = \`<button class="btn btn-success" onclick="importParsedPlayers(\${JSON.stringify(players).replace(/</g,'\\\\u003c')}, '\${source}')"><i class="fas fa-save"></i> Import \${players.length} Players</button>\`
}

async function importParsedPlayers(players, source) {
  const teamId = document.getElementById('ingest-target-team').value
  if (teamId) {
    const d = await api('POST', \`/teams/\${teamId}/import\`, { players })
    toast('Imported ' + d.count + ' players!', 'ok')
  } else {
    const teamName = prompt('Create new team — enter name:')
    if (!teamName) return
    const td = await api('POST', '/teams', { name:teamName.trim(), sport:'basketball', color:'#6C63FF', logoInitials:teamName.substring(0,2).toUpperCase() })
    if (td.team) {
      const d = await api('POST', \`/teams/\${td.team.id}/import\`, { players })
  ...
  document.getElementById(\`\${source}-import-actions\`).style.display = 'none'
}

// OCR (simulated — extracts from pasted text if image text is readable)
function handleOCRFile(event) {
  ...
  reader.onload = e => {
    ...
    // Simulate OCR note (real OCR would require server-side Tesseract or cloud API)
    document.getElementById('ocr-result').innerHTML = \`
      <div style="background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center">
        ...
        <p style="font-size:13px;color:var(--text2)">Image uploaded! For best results, copy the roster text from your document and use <strong style="color:var(--text)">CSV/Text</strong> ingestion.</p>
        <p style="font-size:12px;color:var(--text3);margin-top:6px">Full OCR (AI-powered image reading) is available in the Pro tier with Cloudflare AI Vision integration.</p>
...
async function loadStatsForGame() {
  ...
  content.innerHTML = buildStatsTable(game, stats, cfg)
...
async function loadAnalytics() {
  ...
  content.innerHTML = \`
    <div class="analytics-grid">
      ...
      <div class="insight-card">
        <div class="insight-title"><i class="fas fa-bolt"></i> Pts per Minute <span style="font-size:11px;color:var(--text3);font-weight:400">(min. 5 min played)</span></div>
        \${efficient.length ? \`<div class="perf-bar-wrap">
          \${efficient.slice(0,5).map((p,i) => \`<div class="perf-bar-row">
            <div class="perf-bar-label">\${esc(p.player.name)}</div>
        ...
        </div>\` : '<p style="color:var(--text3);font-size:12px">Not enough minutes data yet.</p>'}
      </div>

      <!-- Court Time (Top by Minutes) -->
      <div class="insight-card">
        ...
        \${highUsage[0]?.min > 30 ? '<p style="font-size:11px;color:var(--danger);margin-top:10px;padding:6px;background:rgba(255,71,87,0.08);border-radius:6px">⚠️ '+esc(highUsage[0].player.name)+' may be overplayed ('+highUsage[0].min.toFixed(0)+'+ min). Consider more rotation.</p>' : ''}
      </div>
...
function generateInsights(agg, team, gamesPlayed) {
  ...
  if (topScorer && (topScorer.pts||topScorer.goals||0) > 0)
    insights.push(\`<div style="padding:8px 12px;background:rgba(108,99,255,0.08);border-radius:8px;border-left:3px solid var(--primary)">🏅 <strong>\${esc(topScorer.player.name)}</strong> leads in scoring with \${topScorer.pts||topScorer.goals||0} total points across \${gamesPlayed} game\${gamesPlayed!==1?'s':''}.</div>\`)

  if (efficient && efficient !== topScorer && efficient.min > 2)
    insights.push(\`<div style="padding:8px 12px;background:rgba(0,212,170,0.08);border-radius:8px;border-left:3px solid var(--success)">⚡ <strong>\${esc(efficient.player.name)}</strong> is the most efficient scorer at \${((efficient.pts||efficient.goals||0)/efficient.min).toFixed(2)} pts/min — consider more court time.</div>\`)
...
function toast(msg, type='ok') {
  ...
  t.innerHTML = \`<i class="fas \${type==='ok'?'fa-check-circle':type==='err'?'fa-circle-exclamation':'fa-info-circle'}"></i>\${esc(msg)}\`
  box.appendChild(t)
  setTimeout(() => { t.style.opacity='0'; t.style.transform='translateX(40px)'; t.style.transition='all 0.3s'; setTimeout(()=>t.remove(),300) }, 2500)
}
